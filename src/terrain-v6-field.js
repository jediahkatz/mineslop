...
import { clamp, fractal, mix, noise, smooth } from "./noise.js";
import {
  selectV5LandBiome, V5_BIOME_SIZE, V5_MUSHROOMS, v5Owner, v5Ramp as ramp,
} from "./terrain-v5-biomes.js";
import { rememberV5, V5_LIMITS, v5InBounds } from "./terrain-v5-config.js";
import { createV5Field } from "./terrain-v5-field.js";
import { V6_COAST, v6CoastalHeight, v6TerraceHeight } from "./terrain-v6-policy.js";
...
// Frozen v5 cave-mouth and seabed policy, copied here so future corrections
// cannot reinterpret the old field. Material identity is not blurred.
function openingsAt(x, z, landTop, dry, salt) {
  ...
  return { top, openings: openings.length
...
export function createV6Field(salt, dimension, counters) {
  // Nether/End geometry and mushroom anchors are unchanged frozen policies,
  // not v5 generators. The enclosing factory/context always remains v6.
  const frozen = createV5Field(salt, dimension, counters);
  if (dimension !== "overworld") return frozen;
  const columns = new Map(), regions = new Map();
  ...
  function region(gx, gz) {
    ...
    return rememberV5(regions, key,
      v5Owner(gx, gz, salt, temperatureAt, moistureAt, dimension), V5_LIMITS.regions);
  }

  function regional(x, z) {
    const wx = x + (noise(x / 470, z / 470, salt ^ 1283) - 0.5) * 112;
    const wz = z + (noise(x / 470, z / 470, salt ^ 1993) - 0.5) * 112;
    const gx = Math.floor(wx / V5_BIOME_SIZE), gz = Math.floor(wz / V5_BIOME_SIZE);
    const candidates = [];
    ...
    for (let dz = -1; dz <= 1; dz++) for (let dx = -1; dx <= 1; dx++) {
      ...
      if (distance < minimum) { minimum = distance; nearest = entry; }
    }
    let lift = 0, relief = 0, swamp = 0, dune = 0, terrace = 0, spire = 0, weight = 0;
    const woodland = [];
    for (const [entry, distance] of candidates) {
      ...
      dune += Number(entry.id === "desert") * w;
      terrace += Number(entry.id === "badlands" || entry.id === "savanna_plateau") * w;
      spire += Number(entry.id === "badlands" && entry.variant < 0.14) *
        (1 - ramp(distance, 64, 184)) * w;
      if (w > 0) woodland.push({ id: entry.id, weight: w });
      weight += w;
    }
    return {
      id: nearest.id, variant: nearest.variant, core: 1 - ramp(minimum, 64, 184),
      lift: lift / weight, relief: relief / weight, swamp: swamp / weight,
      dune: dune / weight, terrace: terrace / weight, spire: spire / weight,
      woodland: Object.freeze(woodland.map((entry) =>
        Object.freeze({ id: entry.id, weight: entry.weight / weight }))),
    };
  }

  function mushroomAt(x, z) {
    const gx = Math.floor(x / V5_MUSHROOMS.spacing), gz = Math.floor(z / V5_MUSHROOMS.spacing);
    let strength = 0;
    for (let dz = -1; dz <= 1; dz++) for (let dx = -1; dx <= 1; dx++) {
      const island = frozen.mushroomIsland(gx + dx, gz + dz);
      if (!island) continue;
  ...
  function overworld(x, z) {
    ...
    let id = field.id, oceanHeight = 0, landHeight = 0, trench = 0, channel = 0;
    if (continental < V6_COAST.land) {
      ...
      oceanHeight = SEA - 1 - shelf * (14 + detail * 8) -
        basin * (55 + noise(wx / 87, wz / 87, salt ^ 35591) * 26) - trench * 27 + small;
      const island = mushroomAt(x, z);
      if (island > 0) {
        oceanHeight = mix(oceanHeight, SEA + 9 + detail * 9, island);
        if (oceanHeight > SEA + 1) id = "mushroom_fields";
      }
    }
    if (continental > V6_COAST.ocean) {
      const inland = ramp(continental, 0.44, 0.58);
      landHeight = SEA + 1 + inland * (
      ...
      landHeight += cliff * 27 * ramp(continental, 0.44, 0.459) *
        (1 - ramp(continental, 0.49, 0.55));
      landHeight += field.dune * inland * (1 - Math.abs(Math.sin(
        wx * 0.06 + wz * 0.024 + noise(wx / 150, wz / 150, salt ^ 193) * 9
      ))) ** 2 * 7;
      landHeight = mix(landHeight, v6TerraceHeight(landHeight), 0.76 * inland * field.terrace);
      landHeight += noise(wx / 24, wz / 24, salt ^ 1913) ** 4 * 28 * inland * field.spire;
      ...
      landHeight = mix(landHeight, SEA + 6 + detail * 3, valley * 0.55);
      landHeight = mix(landHeight, SEA - 7 + detail * 3, Math.max(channel, lake));
      landHeight = mix(landHeight, SEA - 1.8 + detail * 5.4, smooth(field.swamp) * 0.98);
    }
    let height = v6CoastalHeight(continental, oceanHeight, landHeight);
    ...
    return {
      ...
      strataOffset: category === "badlands" ? Math.floor(noise(x / 191, z / 191, salt ^ 1811) * 7) : 0,
  ...
  function sampleColumn(x, z) {
    ...
    return rememberV5(columns, key, Object.freeze(overworld(x, z)), V5_LIMITS.columns);
  }
  return {
    sampleColumn, mushroomIsland: frozen.mushroomIsland,
    get cacheSizes() {
      return { columns: columns.size, regions: regions.size, mushrooms: frozen.cacheSizes.mushrooms };
    },
  };
}
