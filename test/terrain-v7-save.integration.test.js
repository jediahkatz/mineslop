...
const seed = "  v7 / raw:🌲 seed  ";
const snapshot = () => ({
  version: 3,
  world: {
    version: 3, generatorVersion: 7, seed, dimension: "overworld",
    edits: [
...
test("explicit v7 saved-version restoration preserves cells; 1–6 never migrate", async (t) => {
  ...
  for (const generatorVersion of [1, 2, 3, 4, 5, 6, 7, 4, 7, 5, 7, 6]) {
  ...
  assert.equal(world.loadEdits({ ...snapshot().world, generatorVersion: 8 }), false);
});

test("v7 file and IndexedDB roundtrips keep archive3/chunk2 schemas", async () => {
  ...
  await first.save(original); await first.close();
  const second = new WorldStorage({ indexedDB }), restored = await second.load();
  assert.equal(restored.world.generatorVersion, 7);
  assert.deepEqual(new Set(restored.world.edits.map(JSON.stringify)),
    new Set(original.world.edits.map(JSON.stringify)));
  await second.close();
  for (const generatorVersion of [1, 2, 3, 4, 5, 6, 7]) {
  ...
  const future = snapshot(); future.world.generatorVersion = 8;
  assert.throws(() => parseWorldFile(JSON.stringify(future)), /version/);
});

test("layout-v1 contexts alternate4–7; same IDs never permit cross-version maps or sidecars", () => {
  const id = `structure:v1:${encodeURIComponent(JSON.stringify(seed))}:overworld:dungeon:0:0`;
  for (const version of [4, 5, 6, 7, 4, 7, 5, 7, 6]) {
    ...
    const target = { seed, generatorVersion: version, dimension: "overworld", structureId: id, x: 10, y: -40, z: 10 };
    ...
    const pearls = { version: 1, seed, generatorVersion: version, ownerId: "local-player",
      life: 0, cooldown: 0, randomState: 42, nextId: 2, accumulator: 0, projectiles: [{
    ...
    for (const other of [4, 5, 6, 7].filter((entry) => entry !== version)) {
  ...
  for (const version of [1, 2, 3, 8, "7"])
