...
class DecodedDenseSolver extends DenseSolver {
  begin(sources) {
    super.begin(sources.map((s) => s?.values && s.palette ?
      { ...s, palette: undefined, values: Uint32Array.from(s.values, (v) => s.palette[v]) } : s));
  }
}
...
test("negative-coordinate pages, edge aprons, mutations and GPU restore equal dense reference", (t) => {
  ...
  dense.solver = new DecodedDenseSolver();
  const at = { x: -24, y: 8, z: -8 };
  const equivalent = () => {
    settleLight(optimized, world, at, 1); settleLight(dense, world, at, 1);
    assert.deepEqual(optimized.valid.map((v) => Number(v !== 0)), dense.valid.map((v) => Number(v !== 0)));
    for (const [key, entry] of optimized.cache) assert.deepEqual(entry.values, dense.cache.get(key).values, key);
  ...
  test(`field ${interruption} during generation rollover preserves dense-reference lighting`, (t) => {
    ...
    for (let edit = 0; edit < 130 && !optimized.solver.resetPending; edit++) {
      ...
      do {
        ...
        optimized.store.flush(renderer);
